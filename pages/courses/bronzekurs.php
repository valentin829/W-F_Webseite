...
<body>
    ...
    function checkAndDeleteParticipants($result, $conn) {
        while ($row = $result->fetch_assoc()) {
            ...
            if ($completed) {
                // Löscht den Teilnehmer aus der bronzekurs-Tabelle
                $deleteSql = "DELETE FROM bronzekurs WHERE TeilnehmerID = ?";
                ...
                if ($deleteResult) {
		            ...
		            $updateSql = "UPDATE mitglieder_db SET Historie = CONCAT(Historie, '\n<hr> Bronzekurs - abge. " . date('d.m.Y') . "<br>') WHERE Vorname = ? AND Nachname = ?";
